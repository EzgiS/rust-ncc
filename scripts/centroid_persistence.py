...
def calculate_centroid_info(centroids_per_c_per_s):
    ...
    path_dist_centroid = np.sum(dist_group_centroid_per_s[:])
    print("path_dist_group: {}".format(path_dist_centroid))
    end_to_end_dist_centroid = np.linalg.norm(avg_centroid_per_s[-1] -
                                              avg_centroid_per_s[0])
    print("end_to_end_dist_group: {}".format(end_to_end_dist_centroid))
    plt.plot(avg_centroid_per_s[:, 0], avg_centroid_per_s[:, 1], color="r")
    plt.show()

    persistence = end_to_end_dist_centroid / path_dist_centroid
    print("persistence length: {}".format(persistence))
...
exp_jsons = ["four_cell_adh_0", "four_cell_adh_5"]
...
centroids_per_c_per_s_per_seed_per_experiment = \
    [[rust_dat.centroids_per_c_per_s
     ...
     in seeded_experiments_per_json]
